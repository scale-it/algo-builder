...
export function mkAccounts(input: AccountDef[]): rtypes.Account[] {
	...
	if (!errs.isEmpty()) {
		throw new BuilderError(ERRORS.ACCOUNT.MALFORMED, { errors: errs.toString() });
	}
	return accounts;
}
...
function parseMnemonic(mnemonic: string): AccountSDK {
	try {
		...
		if (e instanceof Error) {
			throw new BuilderError(ERRORS.ACCOUNT.WRONG_MNEMONIC, { errmsg: e.message }, e);
		}
		throw e;
	}
}
...
export function loadAccountsFromEnv(): rtypes.Account[] {
	...
	if (algobAccountsString) {
		...
		try {
			...
			throw new BuilderError(ERRORS.ACCOUNT.MALFORMED, {
	...
	for (const account of algobAccounts) {
		if (account.name === undefined) {
			throw new BuilderError(ERRORS.ACCOUNT.FIELD_REQUIRED, {
				errors:
					"Field account name must be defined and not empty in " + JSON.stringify(account),
			});
		}
		if (account.mnemonic === undefined) {
			throw new BuilderError(ERRORS.ACCOUNT.FIELD_REQUIRED, {
				errors:
					"Field mnemonic string must be defined and not empty in " + JSON.stringify(account),
			});
		}
		if (account.name === "") {
			throw new BuilderError(ERRORS.ACCOUNT.FIELD_REQUIRED, {
	...
	async loadKMDAccounts(kcfg: KmdCfg): Promise<rtypes.Account[]> {
		...
		try {
			...
			if (e.code === "ECONNREFUSED") {
				throw new BuilderError(ERRORS.KMD.CONNECTION, { ctx: e }, e);
			}
			if (e instanceof Error) {

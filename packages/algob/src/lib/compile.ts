...
import type { ASCCache, PyASCCache, ScParamsMap } from "../types";
...
export class CompileOp {
   ...
   */
  async ensureCompiled (filename: string, force?: boolean, scTmplParams?: ScParamsMap): Promise<ASCCache> {
    if (force === undefined) {
      force = false;
    }
...
export class PyCompileOp {
   ...
   * @param scTmplParams: Smart contract template parameters (used only when compiling PyTEAL to TEAL)
   */
  async ensureCompiled (filename: string, force?: boolean, scTmplParams?: ScParamsMap): Promise<PyASCCache> {
    if (!filename.endsWith(pyExt)) {

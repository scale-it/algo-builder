...
export interface AlgoOperator {
  ...
  fundLsig: (name: string, flags: FundASCFlags, payFlags: rtypes.TxParams,
    txWriter: txWriter, scParams: LogicSigArgs, scTmplParams?: ScParamsMap) => Promise<LsigInfo>
  deploySSC: (
  ...
  ensureCompiled: (name: string, force?: boolean, scTmplParams?: ScParamsMap) => Promise<ASCCache>
}
...
export class AlgoOperatorImpl implements AlgoOperator {
  ...
  async fundLsig (
    ...
    scTmplParams?: ScParamsMap): Promise<LsigInfo> {

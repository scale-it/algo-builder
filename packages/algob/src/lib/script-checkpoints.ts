...
export function appendToCheckpoint(
	...
	for (const assetName of allAssetNames) {
		if (
			(orig.asa.get(assetName) &&
				!deepEqual(orig.asa.get(assetName), append.asa.get(assetName))) ??
			(orig.app.get(assetName) &&
				!deepEqual(orig.app.get(assetName), append.app.get(assetName)))
		) {
			throw new BuilderError(ERRORS.BUILTIN_TASKS.CHECKPOINT_ERROR_DUPLICATE_ASSET_DEFINITION, {
				assetName: assetName,
			});
		}
	}
...
export class CheckpointRepoImpl implements CheckpointRepo {
	...
	private _ensureRegister(
		map: Map<string, Map<number, rtypes.AppInfo>>,
		name: string,
		info: rtypes.AppInfo
	): void {
...
export function persistCheckpoint(scriptName: string, checkpoint: Checkpoints): void {
	...
	fs.writeFileSync(scriptPath, YAML.stringify(checkpoint));
}
			...
			case "appl": {
				...
				if (res) {
					const sscInfo: rtypes.AppInfo = {
						...
						applicationAccount: getApplicationAddress(
							Number(txConfirmation["application-index"])
						),
						confirmedRound: Number(txConfirmation["confirmed-round"]),
...
export function toSSCMap<T>(obj: {
	[name: string]: { [timestamp: string]: T };
...
function convertCPValsToMaps(cpWithObjects: Checkpoint): Checkpoint {
	...
	cpWithObjects.app = toSSCMap(cpWithObjects.app as any);
...
function findCheckpointsRecursive(): string[] {
	return lsTreeWalk(ensureCheckpointsPath()).filter((filename) =>
		filename.endsWith(checkpointFileSuffix)
	);
}
...
export function loadCheckpointsRecursive(): CheckpointRepo {
	return findCheckpointsRecursive().reduce((out: CheckpointRepo, filename: string) => {
		return out.mergeToGlobal(loadCheckpointByCPName(filename), toScriptFileName(filename));
	}, new CheckpointRepoImpl());
}

export function loadCheckpointsIntoCPData(

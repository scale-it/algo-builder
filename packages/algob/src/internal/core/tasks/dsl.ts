...
import { OverriddenTaskDefinition, SimpleTaskDefinition } from "./task-definitions";

/**
 * This class defines the DSL used in Builder config files
...
export class TasksDSL {
	...
	private _addTask<ArgT extends TaskArguments>(
		...
		if (parentTaskDefinition !== undefined) {
			taskDefinition = new OverriddenTaskDefinition(parentTaskDefinition, isInternal);
		} else {

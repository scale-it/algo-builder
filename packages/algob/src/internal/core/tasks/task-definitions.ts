...
export class SimpleTaskDefinition implements TaskDefinition {
	 ...
	 */
	constructor(public readonly name: string, public readonly isInternal: boolean = false) {
	...
	public addParam<T>(
		...
		if (type === undefined) {
			...
			if (typeof defaultValue !== "string") {
				throw new BuilderError(ERRORS.TASK_DEFINITIONS.DEFAULT_VALUE_WRONG_TYPE, {
					paramName: name,
					taskName: this.name,
				});
			}
   ...
   */
	...
	private _validateNameNotUsed(name: string): void {
		...
		if (Object.keys(ALGOB_PARAM_DEFINITIONS).includes(name)) {
			throw new BuilderError(ERRORS.TASK_DEFINITIONS.PARAM_CLASHES_WITH_ALGOB_PARAM, {
				paramName: name,
				taskName: this.name,
			});
		}
	 ...
	 */
	private _validateNoMandatoryParamAfterOptionalOnes(name: string, isOptional: boolean): void {
		if (!isOptional && this._hasOptionalPositionalParam) {
			throw new BuilderError(ERRORS.TASK_DEFINITIONS.MANDATORY_PARAM_AFTER_OPTIONAL, {
				paramName: name,
				taskName: this.name,
			});
		}
	...
	private _validateParamNameCasing(name: string): void {
		...
		if (match === null) {
			throw new BuilderError(ERRORS.TASK_DEFINITIONS.INVALID_PARAM_NAME_CASING, {
				paramName: name,
				taskName: this.name,
			});
		}
	...
	private _validateNoDefaultValueForMandatoryParam(
		...
		if (defaultValue !== undefined && !isOptional) {
			throw new BuilderError(ERRORS.TASK_DEFINITIONS.DEFAULT_IN_MANDATORY_PARAM, {
				paramName: name,
				taskName: this.name,
			});
		}

...
async function checkTemplateExists(
	...
	if (fse.existsSync(templatePath)) {
		...
		console.log(
			chalk.red(
				`Error occurred: template "${templateName}" does not exist in ${ALGOB_DAPP_TEMPLATES_GIT_REMOTE}`
			)
		);
...
export async function unbox({
	...
	if (templateName === undefined) {
		console.log(
			`Template name not passed: using default template ${chalk.green(DEFAULT_DAPP_TEMPLATE)}`
		);

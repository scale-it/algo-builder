...
export class ArgumentsParser {
		...
		for (let i = 0; i < rawCLAs.length; i++) {
			...
			if (taskName === undefined) {
				...
				if (!this._isCLAParamName(arg, paramDefs)) {
					throw new BuilderError(ERRORS.ARGUMENTS.UNRECOGNIZED_COMMAND_LINE_ARG, {
						argument: arg,
					});
				}
			} else {
	...
	public parseTaskArguments(taskDefinition: TaskDefinition, rawCLAs: string[]): TaskArguments {
		const { paramArguments, rawPositionalArguments } = this._parseTaskParamArguments(
			taskDefinition,
			rawCLAs
		);

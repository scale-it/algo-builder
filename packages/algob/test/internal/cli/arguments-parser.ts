...
import { boolean, int, string } from "../../../src/internal/core/params/argument-types";
import {
...
import { RuntimeArgs, TaskArguments, TaskDefinition } from "../../../src/types";
...
describe("ArgumentsParser", function () {
	...
	describe("algob arguments", function () {
		...
		it("should parse a algob argument", function () {
			...
			assert.equal(
				0,
				argumentsParser._parseArgumentAt(rawCLAs, 0, ALGOB_PARAM_DEFINITIONS, runtimeArgs)
			);
			assert.equal(runtimeArgs.showStackTraces, true);
			assert.equal(
				2,
				argumentsParser._parseArgumentAt(rawCLAs, 1, ALGOB_PARAM_DEFINITIONS, runtimeArgs)
			);
			assert.equal(runtimeArgs.network, "local");
		});

...
import { AlgobDeployerRunMode } from "../../../src/internal/deployer";
...
describe("Scripts runner", function () {
  ...
  it("Should pass params to the script", async function () {
    await runScript("./scripts/params-script.js", mkAlgobEnv(), new AlgobDeployerRunMode(new FakeDeployer(), new TxWriterImpl('')));
  ...
  it("Should run the script to completion", async function () {
    ...
    await runScript("./scripts/async-script.js", mkAlgobEnv(), new AlgobDeployerRunMode(new FakeDeployer(), new TxWriterImpl('')));
  ...
  it("Exception shouldn't crash the whole app", async function () {
    await expectBuilderErrorAsync(
      async () => await runScript("./scripts/failing-script.js", mkAlgobEnv(), new AlgobDeployerRunMode(new FakeDeployer(), new TxWriterImpl(''))),
  ...
  it("Nonexistent default method should throw an exception", async function () {
    await expectBuilderErrorAsync(
      async () => await runScript("./scripts/no-default-method-script.js", mkAlgobEnv(), new AlgobDeployerRunMode(new FakeDeployer(), new TxWriterImpl(''))),
  ...
  it("Should wrap error of require", async function () {
    await expectBuilderErrorAsync(
      async () => await runScript("./scripts/failing-script-load.js", mkAlgobEnv(), new AlgobDeployerRunMode(new FakeDeployer(), new TxWriterImpl(''))),
  ...
  it("Should ignore return value", async function () {
    const out = await runScript("./scripts/successful-script-return-status.js", mkAlgobEnv(), new AlgobDeployerRunMode(new FakeDeployer(), new TxWriterImpl('')));

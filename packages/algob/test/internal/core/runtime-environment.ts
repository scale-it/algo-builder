...
describe("Environment", function () {
	...
	beforeEach(function () {
		...
		dsl
			...
			.addOptionalParam("optFileParam", "an opt file param", undefined, types.inputFile)
			.addOptionalParam("optStringParam", "an opt string param", "some default", types.string)
					...
					try {
						...
						if (error instanceof Error) {
							assert.fail(
								error,
								undefined,
								`Should not throw error task ${taskNameToRun} with args ${argsString}. Error message: ${String(
									error.message || error
								)}`
							);
						}

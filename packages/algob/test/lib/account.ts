...
describe("Loading accounts", function () {
	...
	it("mkAccounts handles mnemonic accounts and handles address check", function () {
		...
		assert.throws(
			() => mkAccounts(input),
			"ABLDR400: Please use same address as the one specified in the mnemonic account"
		);
		...
		assert.throws(
			() => mkAccounts(input),
			"ABLDR401: the mnemonic contains a word that is not in the wordlist"
		);
		...
		assert.throws(
			() => mkAccounts(input),
			'ABLDR403: Invalid value "can\'t be empty" for .account_inputs.1.name - Expected a value of type string.'
		);
	});
	...
	it("ENV variables validate ALGOB_ACCOUNTS ", function () {
		...
		const emptyMnemonic = "";
		const badMnemonic =
			"arch day capable one sweet skate outside purse six early learn tuition eagle love breeze pizza loud today popular able divide";

		// fails when account name is empty
		process.env.ALGOB_ACCOUNTS = JSON.stringify([{ name: "", mnemonic: goodMnemonic }]);
		let errmsg =
			"ABLDR404: Field account name must be defined and not empty in " +
			JSON.stringify({ name: "", mnemonic: goodMnemonic });
		...
		errmsg =
			"ABLDR404: Field account name must be defined and not empty in " +
			JSON.stringify({ mnemonic: goodMnemonic });
		assert.throws(() => loadAccountsFromEnv(), errmsg);

		// fails when mnemonic string is empty
		process.env.ALGOB_ACCOUNTS = JSON.stringify([
			{ name: "master", mnemonic: emptyMnemonic },
			{ name: "master", mnemonic: goodMnemonic },
		]);
		errmsg =
			"ABLDR404: Field mnemonic string must be defined and not empty in " +
			JSON.stringify({ name: "master", mnemonic: emptyMnemonic });
		assert.throws(() => loadAccountsFromEnv(), errmsg);
		...
		errmsg =
			"ABLDR404: Field mnemonic string must be defined and not empty in " +
			JSON.stringify({ name: "master" });
		assert.throws(() => loadAccountsFromEnv(), errmsg);

		// fails mnemonic string is bad
		process.env.ALGOB_ACCOUNTS = JSON.stringify([{ name: "master", mnemonic: badMnemonic }]);
		errmsg =
			"ABLDR401: failed to decode mnemonic in " +
			JSON.stringify({ name: "master", mnemonic: badMnemonic });
		assert.throws(() => loadAccountsFromEnv(), errmsg);
	});
});

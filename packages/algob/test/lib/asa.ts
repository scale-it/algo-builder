...
const namedAccount: runtime.types.Account = {
  name: "hi",
  addr: "addr",
  sk: new Uint8Array(1)
};
...
describe("ASA parser", () => {
  it("Should validate correct obj", async () => {
    const valid: runtime.types.ASADefs = {
      A1: {
        total: 1,
        decimals: 0,
        unitName: 'ASA',
    ...
    const parsed = validateASADefs(valid, new Map<string, runtime.types.Account>(), "");
  ...
  it("Should check total to be a number", async () => {
    ...
    expectBuilderError(
      () => validateASADefs(obj, new Map<string, runtime.types.Account>(), ""),
  ...
  it("Should include filename", async () => {
    ...
    expectBuilderError(
      () => validateASADefs(obj, new Map<string, runtime.types.Account>(), "SOME_FILENAME"),
      ERRORS.SCRIPT.ASA_PARAM_PARSE_ERROR,
      "SOME_FILENAME"
    );
  });
  ...
  it("Should check existence of opt-in account name accounts; green path", async () => {
    ...
    validateASADefs(obj, new Map<string, runtime.types.Account>([["hi", namedAccount]]), "");
  });

...
describe("Checkpoint", () => {
	...
	it("Should merge metadata maps", async () => {
		...
		const netCheckpoint2: Checkpoint = cleanupMutableData(
			new CheckpointImpl(
				new Map([
					["key", "updated data"],
					["key2", "data2"],
		...
		netCheckpoint2.asa.set("my asa 2", {
			creator: "creator",
			txID: "",
			assetIndex: 0,
			confirmedRound: 0,
...
describe("CheckpointRepoImpl", () => {
	...
	it("Should crash if duplication is detected between scripts", async () => {
		...
		const cp = new CheckpointRepoImpl().merge(cp1, "script1");
		expectBuilderError(
	...
	it("Should merge checkpoints", async () => {
		...
		const cp = new CheckpointRepoImpl().merge(cp1, "12").merge(cp2, "34").precedingCP;
		assert.deepEqual(cp, {
			network1: {
				timestamp: 1,
				metadata: new Map([["key 1", "data 1"]]),
	...
	it("Should deeply merge the checkpoints", async () => {
		...
		const cpData = new CheckpointRepoImpl().merge(cp1, "12").merge(cp2, "34");

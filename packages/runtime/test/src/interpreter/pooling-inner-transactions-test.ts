	...
	describe("Pooling fee tests in group txns", function () {
		...
		it("Should succeed: when unfunded account opts-in in a group txn", function () {
			...
			if (elonAcc) {
				...
				assert.equal(
					interpreter.runtime.ctx.state.accounts.get(johnAcc.address)?.balance(),
					0n
				);
				...
				assert.equal(
					interpreter.runtime.ctx.state.accounts.get(bobAccount.address)?.balance(),
					1000000n - 3000n
				);
				assert.equal(
					interpreter.runtime.ctx.state.accounts.get(johnAcc.address)?.balance(),
				...
				assert.equal(interpreter.runtime.ctx.state.accounts.get(appAccAddr)?.balance(), 1000n);
				assert.isDefined(
					interpreter.runtime.ctx.state.accounts.get(appAccAddr)?.getAssetHolding(assetID)
				);
			}
		});

		it("Should succeed: when funded account partially covers it's own fee", function () {
			assert.equal(interpreter.runtime.ctx.state.accounts.get(johnAcc.address)?.balance(), 0n);
			...
			// verify deducted fee
			assert.equal(
				interpreter.runtime.ctx.state.accounts.get(appAccAddr)?.balance(),
		...
		it("Should fail: when txn fee is not covered in group txns", function () {
			...
			expectRuntimeError(() => executeTEAL(prog), RUNTIME_ERRORS.TRANSACTION.FEES_NOT_ENOUGH);
		});

...
export class ITxnField extends Op {
	...
	execute(stack: TEALStack): void {
		...
		const subTxCnt = this.interpreter.subTxn.length - 1;
		const updatedSubTx = setInnerTxField(
			this.interpreter.subTxn[subTxCnt],
			this.field,
			valToSet,
			this,
			this.interpreter,
		...
		this.interpreter.subTxn[subTxCnt] = updatedSubTx;
	}
}
...
export class ITxnSubmit extends Op {
	...
	execute(_stack: TEALStack): void {
		...
		const execParams = this.interpreter.subTxn.map((encTx) =>
			encTxToExecParams(
				encTx,
				{
		...
		this.interpreter.runtime.ctx.tx = this.interpreter.subTxn[0];

...
export function assertLen (val: number, expected: number, line: number): void {
  if (val !== expected) {
    throw new TealError(TEAL_ERRORS.TEAL.ASSERT_LENGTH, { exp: expected, got: val, line: line });
  }
}
...
export function assertBase64 (str: string, line: number): void {
  if (!reBase64.test(str)) {
    throw new TealError(TEAL_ERRORS.TEAL.INVALID_BASE64, { val: str, line: line });
  }
}
...
export function assertBase32 (str: string, line: number): void {
  if (!reBase32.test(str)) {
    throw new TealError(TEAL_ERRORS.TEAL.INVALID_BASE32, { val: str, line: line });
  }
}
...
function base64OrBase32 (arg: string, line: number): [string, EncodingType] {
  ...
  throw new TealError(TEAL_ERRORS.TEAL.DECODE_ERROR, { val: arg, line: line });
}
...
export function getEncoding (args: string[], line: number): [string, EncodingType] {
  if (args.length === 1) {
    ...
    throw new TealError(TEAL_ERRORS.TEAL.UNKOWN_DECODE_TYPE, { val: args[0], line: line });
  } else {
    throw new TealError(TEAL_ERRORS.TEAL.UNKOWN_DECODE_TYPE, { val: args[0], line: line });
  }
}

...
def dynamic_fee(ARG_TO, ARG_AMT, ARG_CLS, ARG_FV, ARG_LV, ARG_LEASE):
    """
    ...
    - Suppose the owner of account A wants to send a payment to account ARG_TO, but does not want
    ...
    must be the specified payment transaction from account A to account ARG_TO.

    Parameters:
    ARG_TO: the recipient of the payment from account A
    ARG_AMT: the amount to send from account A to ARG_TO in microAlgos
    ARG_CLS: the account to close out the remainder of account A's funds to after paying ARG_AMT to ARG_TO
    ...
    # Check that the Receiver field is set to be the ARG_TO address.
    recv_field_check = Gtxn[1].receiver() == ARG_TO

    # Check that the CloseRemainderTo field is set to be the ARG_CLS address.
    cls_field_check = Gtxn[1].close_remainder_to() == ARG_CLS

    # Check that the Amount field is set to be the ARG_TO address.

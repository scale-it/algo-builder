...
async function run(runtimeEnv, deployer) {
  ...
  const signedLsig = await deployer.loadMultiSig("sample-text-asc.msig", []);  
  ...
  // Transaction PASS - according to teal logic, amount should be <= 100
  ...
  //Extracting multisig from raw binary file generated by goal
  const rawLsig = await deployer.loadBinaryMultiSig("sample-raw-asc.msig");  

  //Funding multisignature account
  await transferMicroAlgos(deployer, masterAccount, multsigaddr, 10000000, {note: "Funding multisig account"});

  // Transaction PASS - according to teal logic, amount should be <= 100
  await transferAlgo(deployer, { addr: multsigaddr }, bobAccount.addr, 20, rawLsig);

  // Transaction FAIL - according to teal logic, amount should be <= 100
  await transferAlgo(deployer, { addr: multsigaddr }, bobAccount.addr, 200, rawLsig);
}

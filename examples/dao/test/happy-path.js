...
describe('DAO - Happy Paths', function () {
  ...
  describe('Add proposal', function () {
    ...
    it('should save proposal config in proposalLsig for ALGO transfer (type == 1)', () => {
      ...
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'name'), parsing.stringToBytes('my-custom-proposal'));
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'url'), parsing.stringToBytes('www.myurl.com'));
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'url_hash'), parsing.stringToBytes('url-hash'));
      // empty hash_algo must save sha256
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'hash_algo'), parsing.stringToBytes('sha256'));
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'voting_start'), BigInt(votingStart));
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'voting_end'), BigInt(votingEnd));
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'execute_before'), BigInt(executeBefore));
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'type'), BigInt(ProposalType.ALGO_TRANSFER));
    ...
    it('should save proposal config in proposalLsig for ASA transfer (type == 2)', () => {
      ...
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'type'), BigInt(ProposalType.ASA_TRANSFER));
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'from'), parsing.addressToPk(ctx.daoFundLsig.address()));
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'asa_id'), BigInt(ctx.daoAppID));
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'recipient'), parsing.addressToPk(ctx.proposer.address));
    ...
    it('should save proposal config in proposalLsig for message (type == 3)', () => {
      ...
      ctx.syncAccounts();
      assert.deepEqual(ctx.proposalLsigAcc.getLocalState(ctx.daoAppID, 'msg'), parsing.stringToBytes('my-message'));
    });
  });
  ...
  describe('Vote', function () {
    ...
    it('should allow voterA to register deposited tokens as votes', () => {
      ...
      const key = new Uint8Array([...parsing.stringToBytes('p_'), ...parsing.addressToPk(ctx.proposalLsig.address())]);
  ...
  describe('Execute', function () {
    ...
    it('should execute proposal for type == 2 (ASA TRANSFER)', () => {
      ...
      assert.equal(ctx.proposer.getAssetHolding(ctx.govTokenID).amount, beforeProposerHolding + 10n);
    });
  ...
  describe('Withdraw Vote Deposit', function () {
    ...
    it('should allow withdrawal after voting is over', () => {
      ...
      assert.deepEqual(ctx.voterA.getLocalState(ctx.daoAppID, 'deposit'), beforeRecordVoterA - 5n);
      assert.deepEqual(ctx.voterB.getLocalState(ctx.daoAppID, 'deposit'), beforeRecordVoterB - 7n);

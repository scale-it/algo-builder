...
BEGIN
	...
	IF (SELECT NEW.params::jsonb -> 'gs'->gov_asset->'ui') IS NOT NULL THEN
		...
		VALUES (NEW.index, dao_name, NEW.params::jsonb, asset_id);
	END IF;
	RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
GRANT SELECT ON TABLE sigma_daos, sigma_dao_proposals, asset, account_asset, account_app, account TO sigma_dao_user;

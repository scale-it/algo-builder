...
async function run(runtimeEnv, deployer) {
  ...
  //Get Logic Signature
  const logicSignature = await deployer.loadLsig("3-gold-delegated-asc.teal", []);
  ...
  */
  ...
  const kmdMsigAddress = getmultisigAddress(kmdParams.version, kmdParams.threshold, kmdParams.addrs);

  //Extracting multisig from dissembled file generated by goal (goal clerk compile -D <path_to_file>)
  const Msig = await deployer.loadDelegatedMsig("3-gold-msig.multilsig");  
  ...
  */
  ...
  const multsigaddr = getmultisigAddress(mparams.version, mparams.threshold, mparams.addrs);

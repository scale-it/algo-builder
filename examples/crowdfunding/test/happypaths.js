...
describe("Crowdfunding Tests - Happy Paths", function () {
	...
	function setupAppAndEscrow() {
		...
		creator.setGlobalState(
			applicationId,
			"Receiver",
			convert.addressToPk(fundReceiver.address)
		);
	...
	it("Receiver should be able to withdraw funds if Goal is met", () => {
		...
		runtime
			.getAccount(creator.address)
			.setGlobalState(applicationId, "Escrow", convert.addressToPk(escrow.address));
		syncAccounts();

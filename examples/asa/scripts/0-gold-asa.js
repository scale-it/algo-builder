...
async function run (runtimeEnv, deployer) {
  ...
  const promises = [
    executeTransaction(deployer, mkParam(masterAccount, goldOwner.addr, 401000000, { note: message })),
    executeTransaction(deployer, mkParam(masterAccount, john.addr, 401000000, { note: message })),
    executeTransaction(deployer, mkParam(masterAccount, bob.addr, 1000000, { note: message }))];

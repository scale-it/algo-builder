...
async function run (runtimeEnv, deployer) {
  ...
  let lsig = await deployer.loadLogic('htlc.py', [base64ToBytes(wrongSecret)], scTmplParams);
  ...
  lsig = await deployer.loadLogic('htlc.py', [base64ToBytes(secret)], scTmplParams);
  sender = lsig.address();

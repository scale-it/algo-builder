...
The default Runtime round is set to `2` and timestamp to `1`. Runtime doesn't change the round or timestamp - it's up to the user and the test flow to manage the block. To change the block's round and timestamp we need to call `runtime.setBlock(round, timestamp)`. We can retrieve the current round by using `runtime.getRound()` function and current timestamp by using `runtime.getTimestamp()`. <br />
Example:

    runtime.setBlock(5, 10); // set current round to 5 and timestamp to 10
...
Note: Block round and timestamp remains same until user changes it by calling `runtime.setBlock(round, timestamp)`.

...
We use `connectAlgoSigner()` method to connect the AlgoSigner wallet. Next, we create an instance of the `WebMode` class by passing and pass `AlgoSigner` and the chain name. The `web` instance will allow us to send and sign transactions to the blockchain.
 ```js

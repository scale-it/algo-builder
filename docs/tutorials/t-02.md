...
`master` is our default account we use in algob private net. The **secret**, which will protect the escrow funds is `"hero wisdom green split loop element vote belt"`. It will be provided as an argument to the logic signature, which will check if it's hash equal to the stored one (`hash_image`).
